...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(''); setMessage(''); const supabase=createClient(); const emailRedirectTo = mode === 'signup' ? getAuthCallbackUrl() : null; if (mode === 'signup' && !emailRedirectTo) { setBusy(false); setError('Authentication is not configured for this environment.'); return } const result=mode==='signin' ? await supabase.auth.signInWithPassword({email,password}) : await supabase.auth.signUp({email,password,options:{data:{full_name:name},emailRedirectTo:emailRedirectTo!}}); setBusy(false); if(result.error){setError(authErrorMessage(result.error.message));return} if(mode==='signup' && !result.data.session){setMessage('Check your email to confirm your account, then sign in.');return} const {data:{user}}=await supabase.auth.getUser(); if(!user){setError('Your session could not be established. Please try again.');return} router.replace('/dashboard'); router.refresh() }
