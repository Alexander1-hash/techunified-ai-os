...
export default function LoginPage() {
  ...
  async function resendConfirmation(){const address=email.trim();if(!address){setError('Enter your email address first.');return}if(resendSeconds>0||busy)return;setBusy(true);setError('');const {error}=await createClient().auth.resend({type:'signup',email:address,options:{emailRedirectTo:'https://techunified-ai-os.vercel.app/auth/callback'}});setBusy(false);if(error){setError('Unable to resend the confirmation email. Please wait a moment and try again.');return}setResendSeconds(60);setMessage('Confirmation email sent. Please check your inbox.')}
