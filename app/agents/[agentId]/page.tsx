...
export default async function AgentWorkspace({
  ...
  const agent = {
    ...
    model: agentRecord.model || 'Not configured',
    lastActivity: agentRecord.last_activity_at
      ? new Date(agentRecord.last_activity_at).toLocaleDateString()
      : agentRecord.updated_at
        ? new Date(agentRecord.updated_at).toLocaleDateString()
        : 'No activity',
    tasks: 0,
  }
