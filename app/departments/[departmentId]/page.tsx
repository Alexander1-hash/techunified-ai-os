...
import { getServerDepartment } from '@/lib/repositories/workspace-server'
...
export default async function DepartmentWorkspace({ params }: { params: Promise<{ departmentId: string }> }) {
  ...
  const result = await getServerDepartment(departmentId)
  if (!result) return <main className="p-8">Department not found.</main>
  const department = { ...result.department, agents: result.agents.length, active: result.agents.filter((agent) => agent.status === 'Running').length, icon: '◈', status: 'Operational' }
  const departmentAgents = result.agents
