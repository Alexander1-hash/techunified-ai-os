...
export async function POST(request: Request) {
  ...
  const { data: records, error: recordsError } = await supabase.from('business_source_records').select('id,payload,recorded_at').eq('source_id', source.id).eq('organization_id', organizationId)
  if (recordsError) return NextResponse.json({error:'Source records are unavailable. Apply the Brain pipeline migration.'},{status:500})
  const sample = (records ?? []).map((record:any)=>record.payload); const valid = body.mappings.filter((m:any)=>typeof m.fieldName==='string' && METRICS.includes(m.metricName))
