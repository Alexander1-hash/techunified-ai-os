...
export async function getWorkspaceData() {
  ...
  const [
    ...
    supabase
      ...
      .select(
        'id, organization_id, metric_name, metric_value, recorded_at',
      )
      .eq('organization_id', organizationId)
  ...
  const agents = (agentsResult.data ?? []).map((agent) => ({
    ...agent,
    department:
      departmentNames.get(agent.department_id ?? '') ?? 'Unassigned',
  ...
  const departments = (departmentsResult.data ?? []).map((department) => {
    const departmentAgents = (agentsResult.data ?? []).filter(
      (agent) => agent.department_id === department.id,
    )

    return {
      ...department,
      agents: departmentAgents.length,
      active: departmentAgents.filter((agent) =>
  ...
  return {
    ...
    agents,
    departments,
    workflows: (workflowsResult.data ?? []) as WorkspaceWorkflow[],
    documents: (documentsResult.data ?? []) as WorkspaceDocument[],
    activity,
    metrics: (metricsResult.data ?? []) as WorkspaceMetric[],
  }
}
